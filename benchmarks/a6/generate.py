...
def render(op, form, mode, width):
    ...
    elements_in = count * sl
    elements_out = count * dl
    ...
    for i in range(count):
        if op != "VLDS":
            ...
            code += [f"vlds(a{i}, {source_buffer}, {i * sl}, NORM);"]
        if op in BINARY or op in {"VCMP_EQ", "VSEL"}:
            code += [f"vlds(b{i}, ubC, {i * sl}, NORM);"]
    for i in range(count):
        a = f"d{i-1}" if mode == "forwarding" and i else f"a{i}"
        code += [f"// TARGET {i}: {op} ({mode})"]
        if op == "VLDS":
            code += [f"vlds(d{i}, ubB, {i * sl}, NORM);"]
        elif op == "VSTS":
            code += [f"vsts(a{i}, ubA, {i * dl}, NORM_B{db}, out_mask);"]
        elif op in {"VSTUS", "VSTAS"}:
            code += [f"vector_align align{i} = {{}};",
                     f"__ubuf__ {dt} *ptr{i} = ubA + {i * dl};",
            ...
            code += [f"vsstb(a{i}, ubA + {i * dl}, 0, pset_b16(PAT_VL16));"]
        else:
    ...
    if op not in STORES:
        for i in range(count):
            ...
            code += [f"vsts(d{i}, ubA, {i * dl}, NORM_B{db}, out_mask);"]
...
'''
    meta = dict(name=name, op=op, form=form, mode=mode, width=count,
                ...
                target_aliases=aliases(op), expected_target_count=count,
...
def generate(isa, out, widths=(8, 16)):
    ...
    for op, form, extra in inventory:
        ...
        modes = [("latency", 1)] + [("ii", w) for w in widths]
        if self_raw:
            modes.append(("forwarding", 2))
        names = []
        for mode, width in modes:
            meta, cce = render(op, form, mode, width)
        ...
        coverage.append(dict(op=op, form=form, a6_native_extra=extra, cases=names,
                             ...
                             note=note))
    manifest = dict(schema_version=1, isa_sha256=hashlib.sha256(Path(isa).read_bytes()).hexdigest(),
...
def main():
    ...
    p.add_argument("--widths", nargs="+", type=int, default=[8, 16])
    args = p.parse_args()
    if not all(2 <= w <= 32 for w in args.widths):
        p.error("II widths must be 2..32 (bounded UB and register use)")
    m = generate(args.isa, args.output, args.widths)
